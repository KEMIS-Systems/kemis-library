...
export function SplitButton(props: ISplitButtonProps) {
  ...
  const handleSetDropListPosition = useCallback(
    (event: UIEvent | null = null) => {
      try {
        // if(!event) throw new Error('The UIEvent is not valid event');

        if (!SPLIT_LIST_ELEMENT_REF?.current || !SPLIT_BUTTON_ELEMENT_REF?.current)
          throw new Error("Some essential elements ref is no available");
        ...
        if (VIEWPORT_WIDTH <= 0 || VIEWPORT_HEIGHT <= 0)
          throw new Error("The window dimension is not valid");

        if (
          RECT_BUTTON_LEFT >= RECT_LIST.width / 2 &&
          (RECT_BUTTON_LEFT >= RECT_LIST.width ||
            RECT_BUTTON_LEFT + RECT_LIST.width <= VIEWPORT_WIDTH) &&
          VIEWPORT_WIDTH >= RECT_LIST.width
        ) {
        ...
        if (
          RECT_BUTTON_RIGHT >= RECT_LIST.width / 2 &&
          (RECT_BUTTON_RIGHT >= RECT_LIST.width ||
            RECT_BUTTON_RIGHT + RECT_LIST.width <= VIEWPORT_WIDTH) &&
          VIEWPORT_WIDTH >= RECT_LIST.width
        ) {
  ...
  return (
    <>
      <input
        ...
        id={props.buttonName || ""}
        className="peer/DropList hidden"
      />
      <button
